...
export default function VirtualTryOn() {
	...
	const handleGarmentImageSelect = (
		event: React.ChangeEvent<HTMLInputElement>,
	): void => {
		const file = event.target.files?.[0];
		event.target.value = '';
	...
	const handleTryOn = async (): Promise<void> => {
		if (!bodyImage || !garmentImage || !selectedGarmentType) {
			setError(
				'Please upload both body and garment images and select a garment type',
			);
			return;
		}
		...
		<Container maxWidth="lg" sx={{ mb: 4 }}>
			<Box sx={{ my: 4, textAlign: 'center', position: 'relative' }}>
				...
				<Typography variant="h6" color="text.secondary" paragraph>
					Upload your full-body photo and the garmenting item you want
					to try on
				</Typography>
			</Box>
			...
			>
				<Box>
					...
					<UploadBox
						...
						onClick={() =>
							document.getElementById('garment-input')?.click()
						}
					>
						{garmentPreview ? (
							<PreviewImage
						...
						<Paper sx={{ p: 3, height: '100%' }}>
							...
							>
								<Typography color="text.secondary">
									Result will be displayed here after
									processing
								</Typography>
							</Box>
						</Paper>
